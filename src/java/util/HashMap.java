...
public class HashMap<K,V> extends AbstractMap<K,V> implements Map<K,V>, Cloneable, Serializable {
     ...
     * shrinkage.
     */
    // 因为在遵循良好的hash随机分布（泊松分布）的情况下，要达到8个的概率只有万分之一，所以这是时间与空间的考量
     ...
     */
     Node<K,V>[] table;    // 哈希数组（注：哈希数组的容量跟HashMap可以存储的元素数量不是一回事）
    ...
    static class HashMapSpliterator<K, V> {
        ...
        int fence;  size                // one past last index
